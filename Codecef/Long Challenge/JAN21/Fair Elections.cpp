...
{  IOS
   ...
   while(t--){
   	...
   	ll sum=0,sum2=0;
   	for(ll i=0; i<n; i++){cin>>ar1[i];sum+=ar1[i];}
   	for(ll i=0; i<m; i++){cin>>ar2[i];sum2+=ar2[i];}
   sort(all(ar1));
   sort(ar2.rbegin(),ar2.rend());
  
   	ll cnt=0;
   	for(ll i=0; i<ar1.sz(); i++){
   		if(sum>sum2)break;
   		if(ar1[i]<ar2[i]){cnt++; sum-=ar1[i]; sum+=ar2[i]; sum2-=ar2[i]; sum2+=ar1[i];}
   		
   	}
   	if(sum<=sum2)cout<<-1<<nl;
   	else if(sum>sum2)cout<<cnt<<nl;
   	else cout<<-1<<nl;
