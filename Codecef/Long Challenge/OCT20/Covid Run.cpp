...
int decimalToBinary(int N) {     ull B_Number = 0;     int cnt = 0;     while (N != 0) {  int rem =N% 2; 
 ull c = pow(10, cnt);  B_Number += rem * c;   N /= 2;  cnt++;   }   return B_Number; } 
...
//}
 ...
 ll in=n,xx=x; 
 n--;
 if(k==0){if(x==y)cout<<"YES"<<nl;else cout<<"NO"<<nl;}
 else{
 	bool no=true;
    while(true){
     if(x==y){cout<<"YES"<<nl;no=false;break;}
     x=(x+k)%in;
     if(x==xx)break;
      }
  if(no)cout<<"NO"<<nl;
