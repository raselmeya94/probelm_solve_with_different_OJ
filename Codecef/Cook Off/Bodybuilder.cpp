...
int main()
{
	...
	while (test--)
	{
	    ...
	    for(auto &val:va)cin>>val;
	    for(auto &val:vb)cin>>val;
	    ll tention=vb[0],max_tention=vb[0];
	    for(int i=1;i<va.sz(); i++){
	          
	          ll dif=abs(va[i]-va[i-1]);
	          dif*=r;
	          if(tention-dif<0)tention=0;
	          else tention-=dif;
	          tention+=vb[i];
	          max_tention=max(max_tention, tention);
	    }
	    cout<<max_tention<<nl;
