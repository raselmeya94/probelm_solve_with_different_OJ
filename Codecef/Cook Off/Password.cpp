...
{  IOS
	...
	while(t--){
		...
		if(x.sz()<10){cout<<"NO"<<nl;continue;}
		ll cntC=0,cntss=0,cntn=0,cnts=0;
		for(ll i=0; i<x.sz(); i++)if( x[i]>='a' and x[i]<='z' )cntss++;
		for(ll i=1; i<x.sz()-1 ; i++){
			if(x[i]>='A' and x[i]<='Z' )cntC++; 
			if(x[i]-'0'>=0 and x[i]-'0'<=9 )cntn++; 
			if( x[i]=='@' or  x[i]=='#' or  x[i]=='%' or x[i]== '&' or x[i]== '?' )cnts++; 
		}
			
		if (cntC>=1 and cntss>=1 and cntn>=1 and  cnts>=1)cout<<"YES"<<nl;
		else cout<<"NO"<<nl;
